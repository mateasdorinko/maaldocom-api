...
{
    [Fact]
    public async Task HandleAsync_ByIdIsValid_ReturnsMediaAlbum()
    {
        // arrange
        ...
        var query = new GetMediaAlbumDetailQuery(mediaAlbum.Id);
    ...
    public async Task HandleAsync_ByIdNotValid_ReturnsNotFound()
    {
        ...
        var query = new GetMediaAlbumDetailQuery(Guid.NewGuid());
        var handler = new GetMediaAlbumDetailQueryHandler(cacheManager);
